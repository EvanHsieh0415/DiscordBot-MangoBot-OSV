...
def verify_setting(ctx, setType:str, limit:int=''):
    if setType in ['longer', 'digit', 'set_all']:
        ...
        with open('.\\settings\\verification.json', mode='w', encoding='utf8') as verifySetFile:
        ...
        if setType == 'member_all':
            ...
            verifyData = {}
        with open('.\\verification_data\\data.json', mode='w', encoding='utf8') as verifyFile:
...
class verify(Cog_Extension):
    ...
    async def get(self, ctx):
        await ctx.send(get_verifyCode(ctx=ctx))
    
    @verify.command()
    async def enter(self, ctx, code:str):
        await ctx.send(enter_verifyCode(ctx=ctx, enterCode=code))
